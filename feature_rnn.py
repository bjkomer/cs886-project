...
Modified by Brent Komer for use in a class project.
...
def test_features(n_epochs=250, optimizer='cg'):
    ...
    for i in xrange(n_steps):
      for j in xrange(n_seq * n_batches):
    ...
    plt.close('all')
    # Use the test data
    sequence = np.zeros((n_steps, 1, n_in))
    target = np.zeros((n_steps, 1), dtype=np.int)
    for i in xrange(n_steps):
      for j in xrange(1):
        val_in = random.getrandbits(5) # between 0 and 31
    ...
    # blue line will represent true classes
    true_targets = plt.step(xrange(n_steps), target[:, 0],
                            marker='o')

    # show probabilities (in b/w) output by model
    guess = model.predict_proba(sequence[:, 0][:, np.newaxis])
    guessed_probs = plt.imshow(guess.squeeze().T, interpolation='nearest',
                               cmap='gray')
    ax2.set_title('blue: true class, grayscale: probs assigned by model')
...
def test_condensed_vector(n_epochs=250, optimizer='cg'):
    """ Test RNN with feature (softmax) outputs. """
    n_hidden = 10
    n_in = 5
    n_steps = 10
    ...
    for i in xrange(n_steps):
      for j in xrange(n_seq * n_batches):
        vec_in = [ random.getrandbits(1) for k in xrange(5) ]
        for k in xrange(5):
          seq[i,j,k] = vec_in[k]
        val_in = vec_in[0] + vec_in[1] * 2 + vec_in[2] * 4 + vec_in[3] * 8 + vec_in[4] * 16
        # Set up targets
        """
        if val_in < 10:
          targets[i,j] = 0
...
def to_vector( index, length ):
    vec = np.zeros(( length ))
    #vec = np.zeros(( length ), dtype=np.int)
    val = index
    i = 0
    while val > 0:
      vec[i] = val % 2
...
def to_label_dict( lines ):
    word_dict = {}
    vector_dict = {}
    for index, line in enumerate( lines ):
      word_dict[ line.strip() ] = index
      vector_dict[ index ] = line.strip()
    return ( word_dict, vector_dict, len(lines) )

def to_word_vector_dict( lines ):
    word_dict = {}
    vector_dict = {}
    total = len( lines )
    vec_length = 0
    while total > 1:
      ...
      total /= 2.0
    for index, line in enumerate( lines ):
      vec = to_vector( index, vec_length )
      word_dict[ line.strip() ] = vec
      vector_dict[ tuple(vec.tolist()) ] = line.strip()
    return ( word_dict, vector_dict, vec_length )

def process_lines( data_lines ):
    # Convert the data into processed data and labels (removing newlines, etc)
    data = []
    labels = []
    for line in data_lines:
      ...
      if len(cur) == 0:
        continue
      split_line = cur.split(" ")
      data.append( split_line[0] )
      labels.append( split_line[1] )
    return ( data, labels )

#TODO: this might not work for mini-batch
def build_seq_targets( word_dict, label_dict, data_lines, n_in, n_out ):
    data, labels = process_lines( data_lines )
    length = len( data )
    seq = np.zeros(( length, 1, n_in ))
    targets = np.zeros(( length, 1 ), dtype=np.int)
    for i in xrange( length ):
      seq[i,:] = word_dict[ data[i] ]
      targets[i] = label_dict[ labels[i] ]
    return ( seq, targets )

def test_word_features(n_epochs=250, optimizer='cg', max_lines=1000):
    """ Test RNN with feature (softmax) outputs. """
    n_hidden = 10
    n_in = 32 #5
    n_steps = 10
    ...
    plt.close('all')
    # Use the test data
    test_lines = data_lines[:n_test]
    ( sequence, target ) = build_seq_targets( word_dict, label_dict, test_lines,
                                              n_in, n_out )
    fig = plt.figure()
    ax1 = plt.subplot(211)
    plt.plot(seq[:, 0])
    ...
    # blue line will represent true classes
    true_targets = plt.step(xrange(n_test), target[:, 0],
                            marker='o')
